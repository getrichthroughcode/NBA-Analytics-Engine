...
class AdvancedMetricsCalculator:
    ...
    def calculate_offensive_rating(
        ...
        qAST = ((min_played / (team_min / 5)) * (1.14 * ((team_ast - ast) / team_fgm))) + \
               ((((team_ast / team_min) * min_played * 5 - ast) / 
                 ((team_fgm / team_min) * min_played * 5 - fgm)) * 
                (1 - (min_played / (team_min / 5))))
        
        # Points produced
        pprod = (fgm * (1 - 0.5 * ((points - ftm) / (2 * fga)) * qAST)) + \
                (0.5 * (((team_points - team_ftm) - (points - ftm)) / 
                 (2 * (team_fga - fga))) * ast) + \
                (ftm * (1 - (1 - (ftm / fta)) * 0.4))
        
        ortg = 100 * (pprod / possessions)
        return round(ortg, 1)
    ...
    def calculate_defensive_rating(
        ...
        deff = ((team_dreb + team_stl + team_blk) / team_min) * min_played
    ...
    def calculate_box_plus_minus(
        ...
        raw_bpm = (
            ...
            0.631 * blk -
            0.176 * (fga - fgm) -
            0.146 * (fta - (points - 2 * fgm)) -  # FTM approximation
            0.162 * tov
        ) / (min_played / 48)
