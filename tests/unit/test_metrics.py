...
from src.analytics.metrics import AdvancedMetricsCalculator, calculate_advanced_metrics
...
class TestAdvancedMetricsCalculator:
    ...
    def test_calculate_true_shooting_pct_normal(self):
        ...
        result = self.calculator.calculate_true_shooting_pct(points=20, fga=15, fta=4)
    ...
    def test_calculate_true_shooting_pct_zero_attempts(self):
        ...
        result = self.calculator.calculate_true_shooting_pct(points=0, fga=0, fta=0)

        assert result == 0.0

    def test_calculate_true_shooting_pct_perfect(self):
        """Test true shooting percentage with perfect shooting"""
        result = self.calculator.calculate_true_shooting_pct(points=30, fga=10, fta=10)

        assert result > 0

    def test_calculate_effective_fg_pct_normal(self):
        """Test effective field goal percentage with normal values"""
        result = self.calculator.calculate_effective_fg_pct(fgm=10, fg3m=3, fga=20)
    ...
    def test_calculate_effective_fg_pct_zero_attempts(self):
        ...
        result = self.calculator.calculate_effective_fg_pct(fgm=0, fg3m=0, fga=0)

        assert result == 0.0

    def test_calculate_effective_fg_pct_all_threes(self):
        """Test effective field goal percentage with all three-pointers"""
        result = self.calculator.calculate_effective_fg_pct(fgm=5, fg3m=5, fga=10)
...
class TestCalculateAdvancedMetrics:
    ...
    def test_no_box_plus_minus_in_output(self):
        ...
        assert "box_plus_minus" not in result[0], "box_plus_minus should not be in staging data"
    ...
    def test_preserves_all_original_fields(self):
        ...
        assert original_keys.issubset(result_keys), "Some original fields were lost"
